<?php
	...
	if(mysqli_num_rows($querycekjml)>=5){
		...
		if (mysqli_num_rows($querycek)>=1){
			...
			$query=mysqli_query($koneksi,"INSERT INTO transaksi (kode_transaksi,jenis_motor,tanggal,kode_jadwal,username,no_polisi,status) VALUES ('$kode_transaksi','$jenis_motor','$tanggal','$kode_jadwal','$username','$no_polisi','$status')") or die(mysqli_error($koneksi));
